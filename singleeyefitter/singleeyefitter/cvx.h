...
namespace cvx
{
    ...
    inline cv::Mat& line(cv::Mat& dst, cv::Point2f from, cv::Point2f to, cv::Scalar color, int thickness=1, int linetype=CV_AA, int shift=8) {
        auto from_i = cv::Point((int)(from.x * (1<<shift)), (int)(from.y * (1<<shift)));
        auto to_i = cv::Point((int)(to.x * (1<<shift)), (int)(to.y * (1<<shift)));

        cv::line(dst, from_i, to_i, color, thickness, linetype, shift);
        return dst;
    }

    inline void cross(cv::Mat& img, cv::Point2f centre, float radius, const cv::Scalar& colour, int thickness = 1, int lineType = CV_AA, int shift = 8)
    {
        cvx::line(img, centre + cv::Point2f(-radius, -radius), centre + cv::Point2f(radius, radius), colour, thickness, lineType, shift);
        cvx::line(img, centre + cv::Point2f(-radius, radius), centre + cv::Point2f(radius, -radius), colour, thickness, lineType, shift);
    }
    inline void plus(cv::Mat& img, cv::Point2f centre, float radius, const cv::Scalar& colour, int thickness = 1, int lineType = CV_AA, int shift = 8)
    {
